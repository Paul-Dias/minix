...
not_runnable_pick_new:
    if (proc_is_preempted(p))
    {
        ...
        if (proc_is_runnable(p))
        {
            /* For round robin: always enqueue at end, regardless of time left */
            enqueue(p);
        }
    }
...
void proc_no_time(struct proc *p)
{
    ...
    {
        /* For round robin: dequeue and mark for re-enqueue at end */
        RTS_SET(p, RTS_NO_QUANTUM);
        /* this dequeues the process and notifies scheduler */
        notify_scheduler(p);
    }
    else
    {
