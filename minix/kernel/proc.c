...
/* Geração dos numero aleatorio para a loteria */
#include <time.h>
#include <stdlib.h>

srand(time(NULL));

#define TICKETS_BASE 10
...
void proc_init(void)
{
	...
	/* Geracao de numero aleatorio para loteria, como a funcao srand 
	 * só pode ser chamada uma vez, foi colocada aqui na inicializacao.
	 */
	srand(time(NULL));
 ...
 */
  ...
  rp->p_tickets = TICKETS_BASE + (16 - q);
 ...
 */
  ...
  int ticket;			/* bilhete sorteado */
  int soma_tickets = 0;				/* soma dos tickets em tempo de execucao */
  int total_tickets = 0;			/* soma total dos tickets */

  for(q=7; q < (NR_SCHED_QUEUES - 1); q++){		/* itera entre as filas de usuário somando os tickets */
	if(!(rp = rdy_head[q])) {
		continue;
	}
	for(p = rp; p != NIL_PROC; p = p->p_nextready)
		total_tickets += p->p_tickets;
  }
  ...
  ticket = (rand() % total_tickets) + TICKETS_BASE;
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	if(q > 6 && q < 15){	/* loteria apenas para filas de usuario */

		for(p = rp; p != NIL_PROC; p=p->p_nextready){

			soma_tickets += p->p_tickets

			if(ticket <= soma_tickets){		/* encontrou o processo com a soma que dá o ticket */
				rp = p;
				break;
